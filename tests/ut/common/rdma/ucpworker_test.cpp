...
protected:
    ...
    void SetUp() override
    {
        ...
        ucpWorker_ = std::make_shared<UcpWorker>(context_, ucpManager_.get(), 0);
...
TEST_F(UcpWorkerTest, TestRmEmptyMap)
{
    ...
    Status status;
    status = ucpWorker_->RmEpByIp(std::string("aaa"));
...
TEST_F(UcpWorkerTest, TestWriteAndRm)
{
    ...
    status = ucpWorker_->RmEpByIp(std::string("bbb"));
    end = std::chrono::steady_clock::now();
    duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    LOG(INFO) << "RmEpByIp took " << duration.count() << "ms";
    // since we have added "aaa" to map, we should expect the removal to be successful.
    EXPECT_EQ(status, Status::OK());
    EXPECT_EQ(ucpWorker_->remoteEndpointMap_.size(), initial_size);
