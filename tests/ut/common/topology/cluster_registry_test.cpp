...
TEST(ClusterRegistryTest, DecodesPutDeleteEvents)
{
    ...
    ClusterStoreEvent put;
    put.type = ClusterStoreEventType::PUT;
    ...
    ClusterStoreEvent del;
    del.type = ClusterStoreEventType::DELETE;
    ...
    EXPECT_EQ(typed.workerId, "127.0.0.1:7001");
    EXPECT_EQ(
        registry.HandleWorkerEvent({ ClusterStoreEventType::PUT, "/datasystem/ring/", "", 0, 0 }, typed).GetCode(),
        K_NOT_FOUND);
    EXPECT_EQ(registry.HandleWorkerEvent({ ClusterStoreEventType::PUT, "127.0.0.1:7001", "", 0, 0 }, typed).GetCode(),
              K_NOT_FOUND);
}
