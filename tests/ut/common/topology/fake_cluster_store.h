...
class FakeClusterStore final : public IClusterStore {
public:
    Status PutForTest(const std::string &tableName, const std::string &key, const std::string &value)
    {
        ...
        Emit(ClusterStoreEventType::PUT, tableName, key, entry);
        return Status::OK();
    }
    ...
    Status DeleteForTest(const std::string &tableName, const std::string &key)
    {
        ...
        entries_.erase(iter);
        entry.revision = ++revision_;
        Emit(ClusterStoreEventType::DELETE, tableName, key, entry);
        return Status::OK();
    }
...
private:
    ...
    void Emit(ClusterStoreEventType type, const std::string &tableName, const std::string &key, const Entry &entry)
    {
        ...
        event.value = type == ClusterStoreEventType::DELETE ? "" : entry.value;
    ...
    std::vector<ClusterWatchElement> watchKeys_;
