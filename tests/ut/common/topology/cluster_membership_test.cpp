 ...
 */
#include "datasystem/common/topology/membership/cluster_membership.h"
...
public:
    ...
    Status HandleWorkerEvent(const ClusterStoreEvent &event, WorkerWatchEvent &typed) override
    {
        (void)event;
        (void)typed;
...
ClusterStoreEvent MakeEvent(ClusterStoreEventType type, const std::string &workerId, const std::string &value,
                            int64_t revision)
{
...
TEST(ClusterMembershipTest, AppliesExternalStorePutAndDeleteEvents)
{
    ...
    DS_ASSERT_OK(membership.HandleStoreEvent(
        MakeEvent(ClusterStoreEventType::PUT, "127.0.0.1:7002", MakeKeepAliveValue("start"), 3)));
    std::shared_ptr<const MembershipSnapshot> snapshot;
    DS_ASSERT_OK(membership.GetSnapshot(snapshot));
    EXPECT_EQ(snapshot->workers.size(), 2ul);
    EXPECT_EQ(snapshot->workers.at("127.0.0.1:7002").serviceState, WorkerServiceState::START);

    DS_ASSERT_OK(membership.HandleStoreEvent(MakeEvent(ClusterStoreEventType::DELETE, "127.0.0.1:7002", "", 4)));
    DS_ASSERT_OK(membership.GetSnapshot(snapshot));
...
TEST(ClusterMembershipTest, NotReadyBeforeRebuildAndStoppedAfterStop)
{
    ...
    EXPECT_EQ(membership
                  .HandleStoreEvent(
                      MakeEvent(ClusterStoreEventType::PUT, "127.0.0.1:7001", MakeKeepAliveValue(ETCD_NODE_READY), 1))
                  .GetCode(),
              K_NOT_READY);
