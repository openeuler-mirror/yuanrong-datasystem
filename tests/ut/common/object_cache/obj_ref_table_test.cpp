...
TEST_F(ObjRefTableTest, GetAllClientRefSkipsEmptyRefIds)
{
    const auto clientId = ClientKey::Intern("empty-client");
    std::vector<std::string> failedIds;
    std::vector<std::string> firstIds;
    DS_ASSERT_OK(gRefTable_.GIncreaseRef(clientId, {}, failedIds, firstIds));

    std::unordered_map<ImmutableString, std::vector<std::string>> refTable;
    gRefTable_.GetAllClientRef(refTable);
    ASSERT_TRUE(refTable.empty());
}
