...
protected:
    static void FillRing(HashRingPb &ring, std::unordered_map<std::string, std::string> &hostIdMap,
                         const std::string &address = "127.0.0.1:1000")
    {
        auto &worker = (*ring.mutable_workers())[address];
        worker.set_state(WorkerPb_StatePb_ACTIVE);
        worker.add_hash_tokens(100u);
...
TEST_F(HashRingRefresherTest, TestInitialFetchRunsBeforePeriodicThread)
{
    ...
    int fetchCount = 0;
    auto fetch = [&fetchCount](const HostPort &, uint64_t, HashRingPb &ring, std::string &, uint64_t &newVersion,
                               bool &changed, std::unordered_map<std::string, std::string> &hostIdMap) {
        ++fetchCount;
...
TEST_F(HashRingRefresherTest, TestForceRefreshDuringFetchIsNotLost)
{
    ...
    auto fetch = [&](const HostPort &, uint64_t, HashRingPb &ring, std::string &, uint64_t &newVersion,
                     bool &changed, std::unordered_map<std::string, std::string> &hostIdMap) {
...
TEST_F(HashRingRefresherTest, TestSuccessfulRefreshUpdatesWorkerCandidates)
{
    ...
    auto fetch = [&](const HostPort &worker, uint64_t, HashRingPb &ring, std::string &, uint64_t &newVersion,
                     bool &changed, std::unordered_map<std::string, std::string> &hostIdMap) {
        {
            std::lock_guard<std::mutex> lock(mutex);
...
{
    auto router = std::make_shared<client::WorkerRouter>("host-a");
    auto fetch = [](const HostPort &, uint64_t, HashRingPb &, std::string &, uint64_t &, bool &,
