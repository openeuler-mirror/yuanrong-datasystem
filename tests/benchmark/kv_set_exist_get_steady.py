...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="TTL for benchmark keys. 0 means no TTL; default: 300",
    )
    parser.add_argument(
        "--delete-after-read",
        action="store_true",
        help="Delete each key after a successful get to avoid data accumulation.",
    )
...
def run():
    ...
    if args.ttl_second < 0:
        raise ValueError("--ttl-second must be >= 0")
    ...
    client2.init()
    logging.info(
        "Connected: client1(set)=%s:%d, client2(exist/get)=%s:%d, qps=%.2f, ttl_second=%d, delete_after_read=%s",
        args.set_host,
        args.set_port,
        args.read_host,
        args.read_port,
    ...
    try:
        while True:
            ...
            try:
                client1.set(key, value, ttl_second=args.ttl_second)
            ...
            if not goto_sleep:
                try:
                    ...
                    if args.delete_after_read:
                        failed_keys = client1.delete([key])
                        if failed_keys:
                            logging.warning("delete failed key=%s failed_keys=%s", key, failed_keys)
                    success += 1
                except Exception as exc:  # pylint: disable=broad-except
                    get_fail += 1
