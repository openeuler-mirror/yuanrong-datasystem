#!/usr/bin/env bash
# E2E test for kvtest deploy/stop/collect
...
if [[ ! -f kvtest ]]; then
    echo "ERROR: kvtest binary not found. Run build.sh first."
    exit 1
fi

# 6. Clean up any previous instances
python3 deploy_client.py clean "$DEPLOY_CFG" "$KVCLIENT_CFG" 2>/dev/null || true

# === TC1: Deploy ===
echo ""
echo "=== TC1: Deploy ==="
python3 deploy_client.py deploy "$DEPLOY_CFG" "$KVCLIENT_CFG"

# Verify kvclient process on both machines
for m in A B; do
    if check_process "$m" "kvtest"; then
...
echo ""
echo "=== TC3: Stop ==="
python3 deploy_client.py stop "$DEPLOY_CFG" "$KVCLIENT_CFG"
sleep 2
...
echo "=== TC4: Collect ==="
rm -rf collected
python3 deploy_client.py collect "$DEPLOY_CFG" "$KVCLIENT_CFG"
...
for round in 1 2; do
    ...
    for m in A B; do
        ...
        if check_process "$m" "kvtest" && check_process "$m" "datasystem_worker"; then
...
START_TIME=$(date +%s)
python3 deploy_client.py stop "$DEPLOY_CFG" "$KVCLIENT_CFG" 2>&1 || true
END_TIME=$(date +%s)
