...
cleanup() {
    ...
    pkill -f "kvtest.*config_tunnel" 2>/dev/null || true
    # Stop remote kvclient instance
    ssh "$REMOTE_HOST" "pkill -f kvtest" 2>/dev/null || true
    sleep 1
    # Stop local worker
...
scp "$BINARY" "${REMOTE_HOST}:${REMOTE_WORK_DIR}/kvtest"
...
ssh "$REMOTE_HOST" "cd ${REMOTE_WORK_DIR} && chmod +x kvtest && \
    LD_LIBRARY_PATH=${REMOTE_WORK_DIR}/sdk_lib:\$LD_LIBRARY_PATH \
    nohup ./kvtest config_1.json > stdout_1.log 2>&1 &"
sleep 3
...
./build/kvtest config/config_tunnel_local.json > stdout_local.log 2>&1 &
LOCAL_PID=$!
