...
```
                    ...
                    │  └───────────┘                          │
                    └─────────────────────────────────────────┘
                          │ HTTP /notify (JSON: keys + sender + size)
                          ▼
                    ┌─────────────────────────────────────────┐
                    │            Reader 进程                   │
                    │                                         │
...
| 线程 | 数量 | 职责 |
|------|------|------|
| Pipeline Threads | `num_set_threads`（默认 16） | Writer 主循环：构造数据 → 执行 Pipeline → 通知对端 |
| NotifyPool (Writer) | 100 | Writer 发送 HTTP 通知给 Reader |
| NotifyPool (Reader) | 100 | Reader 处理收到的通知，执行 Reader Pipeline |
...
sequenceDiagram
    ...
    Main->>HS: Start() → 监听端口
    Main->>W: Start() → 启动 pipeline 线程（仅 writer）
...
示例（target_qps=100, num_set_threads=4, intervalUs=250000us=40ms/线程）：
```
Thread 0: phase = 0ms     → 从 0ms 位置开始
Thread 1: phase = 10ms    → 从 10ms 位置开始
Thread 2: phase = 20ms    → 从 20ms 位置开始
Thread 3: phase = 30ms    → 从 30ms 位置开始
```
