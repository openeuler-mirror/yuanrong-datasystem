...
配置 `test_mode` 字段选择测试模式。8 种模式对应不同的客户端-Worker 拓扑。
...
| `mixed_local` | localClient (SHM) | localClient (SHM) | 无（并发） | Worker A 同机 |
| `mixed_cross_node` | localClient (SHM) | localClient (SHM) | 可能（hash 分布） | Worker A 同机 |

### mixed_local — 本地混合读写

Set 和 Get 线程在同一个子进程内并发执行，连接同一本地 Worker。通过 `set_ratio` 控制读写比例。
...
```json
{
  ...
  "listen_port": 9000,
  "test_mode": "mixed_local",
  "worker_memory_mb": 4096,
  "num_threads": 16,
  "total_rounds": 10,
  "data_sizes": ["1MB"],
  "set_ratio": 0.7,
  "mixed_key_strategy": "same_keys",
  "cleanup_method": "del"
}
```

16 线程中 11 个做 Set，5 个做 Get，同时并发执行。

### mixed_cross_node — 跨节点混合读写

与 `mixed_local` 相同的线程模型，但 Get 操作可能触发跨节点数据拉取（取决于集群中 key 的 hash 分布）。需要至少 2 个 Worker 节点。

配置同 `mixed_local`，将 `test_mode` 改为 `"mixed_cross_node"`。
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `test_mode` | string | **必填** | 测试模式：`set_local` / `set_remote` / `get_local` / `get_cross_node` / `get_remote_direct` / `get_remote_cross` / `mixed_local` / `mixed_cross_node` |
