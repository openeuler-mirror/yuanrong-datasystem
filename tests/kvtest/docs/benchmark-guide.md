...
| `create_buffer` | `Create(key, size, param, buf)` → `buf.WLatch()` → `buf.MemoryCopy(data, size)` → `buf.UnWLatch()` → `Set(buf)` | 显式 SHM Buffer 路径，可测量 Create + MemoryCopy 开销 |
...
| `set_api must be 'string_view' or 'create_buffer'` | set_api 值非法 | 使用 `"string_view"` 或 `"create_buffer"` |
