...
TEST_F(CoordinatorRuntimeElectionTest, FollowerWithDeletedRaftDataRebuildsFromAuthoritativePeers)
{
    const auto caseDeadline = std::chrono::steady_clock::now() + kCaseBudget;
    const auto allIndexes = BaselineIndexes();
    ...
    ASSERT_NO_FATAL_FAILURE(StartBaselineCluster(caseDeadline, initial));
    ASSERT_TRUE(WaitUntil(
        [this, &allIndexes, &baselineEndpoints, &initial, caseDeadline] {
            LeaderObservation stableLeader;
            return ObserveUniqueServingLeader(allIndexes, stableLeader, caseDeadline, initial.endpoint)
                   && std::all_of(allIndexes.begin(), allIndexes.end(),
        ...
        caseDeadline))
        << FailureDiagnostics("deleted-data baseline committed configuration", allIndexes);
    const auto followers = FollowersOf(initial);
    ASSERT_EQ(followers.size(), kBaselineCoordinatorCount - 1);
    const size_t stoppedFollower = followers.front();
    const std::vector<size_t> survivors{ initial.leaderIndex, followers.back() };
    ...
    const auto followerStopCompletedAt = std::chrono::steady_clock::now();
    ASSERT_EQ(runtimes_[stoppedFollower].active, nullptr);
    ASSERT_TRUE(CanBindLoopbackPort(portLeases_[stoppedFollower].Port()));
    ASSERT_TRUE(HasPersistedData(stoppedFollower));
    LeaderObservation afterStop;
    ASSERT_TRUE(WaitUntil(
        [this, &survivors, &afterStop, &initial, caseDeadline] {
            return ObserveUniqueServingLeader(survivors, afterStop, caseDeadline, initial.endpoint);
        },
        caseDeadline))
        << FailureDiagnostics("deleted-data follower stop isolation", survivors);

    std::error_code error;
    const auto removedEntries = std::filesystem::remove_all(dataRoots_[stoppedFollower], error);
    ASSERT_FALSE(error) << error.message();
    ASSERT_GT(removedEntries, 0U);
    ASSERT_FALSE(std::filesystem::exists(dataRoots_[stoppedFollower], error));
    ASSERT_FALSE(error) << error.message();
    for (const auto index : survivors) {
        ASSERT_TRUE(HasPersistedData(index)) << "Survivor Raft data root changed: " << dataRoots_[index];
    }
    ...
    ASSERT_LT(std::chrono::steady_clock::now() - followerStopCompletedAt,
              std::chrono::milliseconds(kMemberFailureGraceMs))
        << "Deleted-data follower did not restart before membership failure grace";

    bool servingInvariantViolated = false;
    LeaderObservation recovered;
    const bool converged = WaitUntil(
        [this, &allIndexes, &survivors, &initial, stoppedFollower, &baselineEndpoints, &restartProvider,
         &servingInvariantViolated, &recovered, caseDeadline] {
            LeaderObservation survivorLeader;
            if (!ObserveUniqueServingLeader(survivors, survivorLeader, caseDeadline, initial.endpoint)) {
            ...
            return restartProvider->DiscoveryQueryCount() > 0
                   && ObserveUniqueServingLeader(allIndexes, recovered, caseDeadline, initial.endpoint)
                   && HasCommittedConfiguration(stoppedFollower, baselineEndpoints, caseDeadline);
        },
        caseDeadline);

    ASSERT_FALSE(servingInvariantViolated)
        << FailureDiagnostics("deleted-data follower serving invariant", allIndexes);
    ASSERT_TRUE(converged) << FailureDiagnostics("deleted-data follower authoritative rebuild", allIndexes);
    EXPECT_GT(restartProvider->DiscoveryQueryCount(), 0U)
        << "ABSENT follower generation did not query bootstrap Discovery";
    ...
    EXPECT_EQ(bootstrap.response.status_code(), static_cast<int32_t>(K_OK));
    EXPECT_EQ(bootstrap.response.candidate_count(), kBaselineCoordinatorCount);
    EXPECT_TRUE(HasCommittedConfiguration(stoppedFollower, baselineEndpoints, caseDeadline));
    EXPECT_EQ(recovered.endpoint, initial.endpoint);
    EXPECT_LT(std::chrono::steady_clock::now(), caseDeadline);
}
