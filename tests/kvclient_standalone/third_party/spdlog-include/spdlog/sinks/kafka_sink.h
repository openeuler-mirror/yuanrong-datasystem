...
using kafka_sink_st = kafka_sink<ds_spdlog::details::null_mutex>;

} // namespace sinks

template<typename Factory = ds_spdlog::synchronous_factory>
inline std::shared_ptr<logger> kafka_logger_mt(const std::string &logger_name, ds_spdlog::sinks::kafka_sink_config config)
{
    return Factory::template create<sinks::kafka_sink_mt>(logger_name, config);
}

template<typename Factory = ds_spdlog::synchronous_factory>
inline std::shared_ptr<logger> kafka_logger_st(const std::string &logger_name, ds_spdlog::sinks::kafka_sink_config config)
{
    return Factory::template create<sinks::kafka_sink_st>(logger_name, config);
}

template<typename Factory = ds_spdlog::async_factory>
inline std::shared_ptr<ds_spdlog::logger> kafka_logger_async_mt(std::string logger_name, ds_spdlog::sinks::kafka_sink_config config)
{
    return Factory::template create<sinks::kafka_sink_mt>(logger_name, config);
}

template<typename Factory = ds_spdlog::async_factory>
inline std::shared_ptr<ds_spdlog::logger> kafka_logger_async_st(std::string logger_name, ds_spdlog::sinks::kafka_sink_config config)
{
    return Factory::template create<sinks::kafka_sink_st>(logger_name, config);
}
