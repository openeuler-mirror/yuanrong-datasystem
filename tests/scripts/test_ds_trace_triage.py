...
def test_brpc_framework_slow_fields_are_parsed(tmp_path):
    trace_id = "019f807e-b084-774e-a428-84ee16885664"
    log = tmp_path / "brpc-slow.log"
    log.write_text(
        f"2026-07-21T04:33:53.159760 | I | brpc_perf_trace | 192.0.2.10 | 1 | {trace_id} | "
        "[BRPC_RPC_FRAMEWORK_SLOW] trace_id=sample "
        "method=datasystem.WorkerOCService.Get framework_us=20184 e2e_us=20184 "
        "client_req_framework_us=0 remote_processing_us=20184 server_req_queue_us=0 "
        "server_exec_us=0 network_residual_us=0\n",
        encoding="utf-8",
    )
    ...
    rpc = report["dimensions"]["rpc_slow"]["datasystem.WorkerOCService.Get"]
    assert rpc["count"] == 1
    assert rpc["e2e_us"]["p50"] == 20184
...
def test_worker_ip_mapping_is_one_row_per_real_worker(tmp_path):
    trace_id = "019f8072-5c96-7f85-9546-f922bd96bfa8"
    log = tmp_path / "mixed-worker.log"
    log.write_text(
        "\n".join(
            [
                f"/logs/kvclusterworker-0-worker7/kvcache.INFO.log:1:2026-07-20T12:31:00.000000 | I | worker | 192.0.2.7 | 1 | {trace_id} | Get done",
                f"/logs/kvclusterworker-0-worker7/access.log:2:2026-07-20T12:31:00.001000 | I | access | 192.0.2.7 | 1 | {trace_id} | 0 | DS_POSIX_GET | 1000 | 1",
                f"/logs/kvclusterworker-0-master/kvcache.INFO.log:3:2026-07-20T12:31:00.002000 | I | worker | 192.0.2.8 | 1 | {trace_id} | QueryMeta done",
                f"/logs/kvclusterclient-0-worker7_4/ds_client_access.log:4:2026-07-20T12:31:00.003000 | I | access | 192.0.2.70 | 1 | {trace_id} | 0 | DS_KV_CLIENT_GET | 1100 | 1 | {{Object_key:kv_test_0,latencySummary:{{worker.rpc.query_meta:100}}}}",
            ]
        ),
        encoding="utf-8",
    )
    ...
    mapping = report["dimensions"]["worker_ip_mapping"]
    assert mapping == [
        {
            "worker_full_name": "kvclusterworker-0-master",
            "worker_short_name": "master",
            "pod_ip": "192.0.2.8",
        },
        {
            "worker_full_name": "kvclusterworker-0-worker7",
            "worker_short_name": "worker 7",
            "pod_ip": "192.0.2.7",
        },
    ]
    assert len({item["worker_full_name"] for item in mapping}) == len(mapping)
    assert len({item["pod_ip"] for item in mapping}) == len(mapping)
    assert not any("kv_test" in worker for worker in report["dimensions"]["workers"])
...
def test_run_pipeline_writes_intermediate_outputs_and_html_targets(tmp_path):
    ...
    mod = _load_module()
    run_dir = mod.run_pipeline(
        [str(log)], tmp_path / "runs", mod.RunOptions(case_name="set-case", scenario="memory-copy")
    )
    ...
    assert "dim.worker_ip_mapping || []" in html
    assert "workerPodIpKey(worker, podIp)" not in html
...
def test_run_pipeline_keeps_legacy_python_api_compatible(tmp_path):
    trace_id = "019f807e-b084-774e-a428-84ee16885664"
    log = tmp_path / "legacy-api.log"
    log.write_text(
        f"2026-07-21T04:33:53.159760 | I | worker | 192.0.2.10 | 1 | {trace_id} | sample\n",
        encoding="utf-8",
    )

    mod = _load_module()
    keyword_run = mod.run_pipeline(
        [str(log)],
        tmp_path / "keyword-runs",
        case_name="legacy-keyword",
        scenario="compatibility",
        code_ref="legacy-ref",
        force=True,
    )
    positional_run = mod.TraceRunPipeline().run(
        [str(log)],
        tmp_path / "positional-runs",
        "legacy-positional",
        "compatibility",
        "legacy-ref",
        True,
    ...
    keyword_manifest = json.loads((keyword_run / "manifest.json").read_text(encoding="utf-8"))
    positional_manifest = json.loads((positional_run / "manifest.json").read_text(encoding="utf-8"))
    assert keyword_manifest["case_name"] == "legacy-keyword"
    assert keyword_manifest["scenario"] == "compatibility"
    assert keyword_manifest["code_ref"] == "legacy-ref"
    assert positional_manifest["case_name"] == "legacy-positional"
    assert positional_manifest["scenario"] == "compatibility"
    assert positional_manifest["code_ref"] == "legacy-ref"
...
def test_run_pipeline_preserves_raw_extracted_logs_and_reuses_cache(tmp_path):
    ...
    mod = _load_module()
    opts = mod.RunOptions(case_name="cache-case", scenario="round-a")
    first = mod.run_pipeline([str(bundle)], tmp_path / "runs", opts)
    second = mod.run_pipeline([str(bundle)], tmp_path / "runs", opts)
    forced = mod.run_pipeline(
        [str(bundle)], tmp_path / "runs", mod.RunOptions(case_name="cache-case", scenario="round-a", force=True)
    )
...
def test_independent_stage_functions_consume_previous_artifacts(tmp_path):
    ...
    mod = _load_module()
    run_dir = mod.parse_stage(
        [str(log)], tmp_path / "runs", mod.RunOptions(case_name="stage-case", scenario="stage-smoke")
    )
...
def test_publish_site_stage_executes_copy_and_verify_commands(tmp_path, monkeypatch):
    ...
    mod = _load_module()
    run_dir = mod.run_pipeline(
        [str(log)], tmp_path / "runs", mod.RunOptions(case_name="publish-case", code_ref="unit-test")
    )
    calls = []
    ...
    assert Path(calls[0][0][0]).name == "scp"
    assert calls[0][0][1] == str(run_dir / "report.site.html")
    assert calls[0][0][2].startswith("publish-host.example:/srv/site/perf/")
    assert Path(calls[1][0][0]).name == "curl" and calls[1][0][1:2] == ["-fsSI"]
    assert Path(calls[2][0][0]).name == "curl" and calls[2][0][1:4] == ["-fsSL", "-A", "Mozilla/5.0"]
...
def test_publish_site_stage_requires_target_env_for_real_publish(tmp_path, monkeypatch):
    ...
    mod = _load_module()
    run_dir = mod.run_pipeline(
        [str(log)], tmp_path / "runs", mod.RunOptions(case_name="publish-env-case", code_ref="unit-test")
    )
    calls = []
    ...
    with pytest.raises(mod.TraceTriageError, match=mod.PUBLISH_HOST_ENV):
...
def test_publish_site_stage_blocks_oversized_html_before_copy(tmp_path, monkeypatch):
    ...
    mod = _load_module()
    run_dir = mod.run_pipeline(
        [str(log)], tmp_path / "runs", mod.RunOptions(case_name="publish-large", code_ref="unit-test")
    )
    calls = []
    ...
    monkeypatch.setattr(mod.subprocess, "run", fake_run)
    with pytest.raises(mod.TraceTriageError) as exc:
...
def test_trace_run_pipeline_object_exposes_stage_boundaries(tmp_path):
    ...
    run_dir = pipeline.parse(
        [str(log)], tmp_path / "runs", mod.RunOptions(case_name="object-pipeline", code_ref="unit-test")
    )
...
def test_invalid_input_fails_by_default_and_can_be_recorded_as_partial(tmp_path):
    ...
    mod = _load_module()
    with pytest.raises(mod.TraceTriageError, match="Failed to read trace input"):
...
def test_raw_tar_preservation_rejects_absolute_member_paths(tmp_path):
    ...
    with pytest.raises(ValueError, match="Unsafe tar member path"):
        store.prepare_parse_run(
            [str(bundle)], tmp_path / "runs", mod.RunOptions(case_name="unsafe-case", code_ref="ref")
        )
...
def test_raw_tar_preservation_enforces_member_budget(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="Tar member count exceeds"):
        mod.TraceRunStore().prepare_parse_run(
            [str(bundle)], tmp_path / "runs", mod.RunOptions(case_name="budget-case", code_ref="ref")
        )
...
def test_trace_run_store_prepares_parse_run_cache_and_raw_inputs(tmp_path):
    ...
    mod = _load_module()
    store = mod.TraceRunStore()
    store_opts = mod.RunOptions(case_name="store-case", scenario="scenario", code_ref="ref")
    prepared = store.prepare_parse_run([str(bundle)], tmp_path / "runs", store_opts)
    ...
    cached = store.prepare_parse_run([str(bundle)], tmp_path / "runs", store_opts)
...
def test_directory_cache_identity_changes_when_contents_change(tmp_path):
    ...
    mod = _load_module()
    first = mod.run_pipeline([str(input_dir)], tmp_path / "runs", mod.RunOptions(case_name="dir-cache"))
    log.write_text(
        f"2026-07-20T13" + ":" + f"28" + ":" + f"00.000000 | INFO | worker | kvworker-0-worker1 | 1 | {trace_b} | second\n",
        encoding="utf-8",
    )
    second = mod.run_pipeline([str(input_dir)], tmp_path / "runs", mod.RunOptions(case_name="dir-cache"))
...
def test_directory_inputs_are_preserved_in_raw_snapshot(tmp_path):
    ...
    mod = _load_module()
    run_dir = mod.run_pipeline(
        [str(input_dir)], tmp_path / "runs", mod.RunOptions(case_name="dir-preserve")
    )
...
def test_same_basename_inputs_are_preserved_without_overwrite(tmp_path):
    ...
    prepared = mod.TraceRunStore().prepare_parse_run(
        ...
        tmp_path / "runs",
        mod.RunOptions(case_name="same-name", code_ref="ref"),
    )
...
def test_cache_key_changes_when_parser_rules_change(tmp_path):
    ...
    mod = _load_module()
    first = mod.run_pipeline([str(log)], tmp_path / "runs", mod.RunOptions(case_name="rule-cache"))
    ...
    second = pipeline.run([str(log)], tmp_path / "runs", mod.RunOptions(case_name="rule-cache"))
