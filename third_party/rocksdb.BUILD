...
config_setting(
    name = "with_liburing_setting",
    flag_values = {":with_liburing": "true"},
    visibility = ["//visibility:public"],
)
...
feature_defines = select({
    ":with_bzip2_setting": ["BZIP2"],
    "//conditions:default": [],
}) + select({
    ":with_liburing_setting": ["ROCKSDB_IOURING_PRESENT"],
    "//conditions:default": [],
}) + select({
    ":with_lz4_setting": ["LZ4"],
    "//conditions:default": [],
}) + select({
...
expand_template(
    name = "build_version_cc",
    out = "util/build_version.cc",
    substitutions = {
        # Based off https://github.com/facebook/rocksdb/releases/tag/v7.10.2
        "@BUILD_DATE@": "2026-03-29 00:00:00",
        "@GIT_SHA@": "499765e",
        "@GIT_TAG@": "v7.10.2",
        "@GIT_DATE@": "2023-02-23 00:00:00",
        "@GIT_MOD@": "0",
        "@ROCKSDB_PLUGIN_EXTERNS@": "",
        "@ROCKSDB_PLUGIN_BUILTINS@": "",
    },
    template = "util/build_version.cc.in",
)
...
cc_library(
    ...
    hdrs = glob(["**/*.h"]),
    includes = ["include"],
    local_defines = rocksdb_os_defines + feature_defines,
    visibility = ["//visibility:public"],
    deps = select({
        ":with_bzip2_setting": ["@bzip2//:bz2"],
        "//conditions:default": [],
    }) + select({
        ":with_liburing_setting": ["@liburing"],
        "//conditions:default": [],
    }) + select({
        ":with_lz4_setting": ["@lz4//:lz4_hc"],
        "//conditions:default": [],
    }) + select({
        ":with_zlib_setting": ["@zlib"],
        "//conditions:default": [],
