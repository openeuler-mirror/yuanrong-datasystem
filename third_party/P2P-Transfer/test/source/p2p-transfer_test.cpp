...
// Warmup
#ifdef USE_HCCL
    NPU_ERROR(HcclSend(dSendBuff, bufferSizeBytes / sizeof(float), HCCL_DATA_TYPE_FP32, recvRank, hcclComm, stream));
#else
    ...
    for (size_t transferSizeBytes = 128 * 1024; transferSizeBytes <= bufferSizeBytes;
...
#ifdef USE_HCCL
            NPU_ERROR(HcclSend(dSendBuff, transferSizeBytes / sizeof(float), HCCL_DATA_TYPE_FP32, recvRank, hcclComm,
                               stream));
#else
            NPU_ERROR(P2PSend(dSendBuff, transferSizeBytes / sizeof(float), HCCL_DATA_TYPE_FP32, p2pComm, stream));
#endif
        ...
        double bw = transferSizeBytes / time.Value();
...
// Warmup
#ifdef USE_HCCL
    NPU_ERROR(HcclRecv(dRecvBuff, bufferSizeBytes / sizeof(float), HCCL_DATA_TYPE_FP32, sendRank, hcclComm, stream));
#else
    ...
    for (size_t transferSizeBytes = 128 * 1024; transferSizeBytes <= bufferSizeBytes;
        ...
        for (int sample = 0; sample < nSamples; sample++) {
            auto t1 = std::chrono::high_resolution_clock::now();
#ifdef USE_HCCL
            NPU_ERROR(HcclRecv(dRecvBuff, transferSizeBytes / sizeof(float), HCCL_DATA_TYPE_FP32, sendRank, hcclComm,
                               stream));
#else
            NPU_ERROR(P2PRecv(dRecvBuff, transferSizeBytes / sizeof(float), HCCL_DATA_TYPE_FP32, p2pComm, stream));
#endif
            NPU_ERROR(aclrtSynchronizeStream(stream));
            auto t2 = std::chrono::high_resolution_clock::now();
            const std::chrono::duration<double, std::milli> ms = t2 - t1;
        ...
        NPU_ERROR(aclrtMemcpy(hostBuff, transferSizeBytes, dRecvBuff, transferSizeBytes, ACL_MEMCPY_DEVICE_TO_HOST));
        Verify(hostBuff, transferSizeBytes / sizeof(float));
...
int Benchmark()
{
    ...
    int nDevices = 2;
    size_t bufferSizeBytes = static_cast<size_t>(4) * 1024 * 1024 * 1024;
