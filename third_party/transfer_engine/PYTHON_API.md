...
- `third_party/transfer_engine/python/yr/datasystem/__init__.py`

- the wheel distribution name is `openyuanrong_transfer_engine`
- the Python import path remains `yr.datasystem`

Binding implementation:

- `third_party/transfer_engine/src/python/py_transfer_engine.cpp`
...
- `Status`: `StatusCode.kOk` on success; otherwise an error status

Notes:

- `device_name` is parsed internally and its numeric suffix is stored as the engine `device_id`
- malformed `device_name` returns `StatusCode.kInvalid`
...
`Status` is the common return type for `TransferEngine` methods.
...
- `StatusCode`: The status code of the result
...
- `third_party/transfer_engine/tests/python/st/test_python_api_st.py`
- `third_party/transfer_engine/tests/python/smoke/test_python_api_smoke.py`
...
- invalid `device_name`, such as `"gpu:0"` or `"0"`, returns `StatusCode.kInvalid`
- transfer before `initialize()` returns `StatusCode.kNotReady`
- empty batch input returns `StatusCode.kInvalid`
- mismatched batch list length returns `StatusCode.kInvalid`
- missing registered region may return `StatusCode.kNotFound`
- unauthorized or invalid remote read may return `StatusCode.kNotAuthorized`
...
- The wheel distribution name is `openyuanrong_transfer_engine`, but the installed Python import path remains `yr.datasystem`.
