...
# Real NPU dual-process validation based on bundled third-party P2P-Transfer benchmark.
# Note: current benchmark target p2p-transfer_test uses fixed device ids: sender=3, receiver=4.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TE_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
P2P_ROOT="${TE_ROOT}/third_party/P2P-Transfer"
P2P_BUILD_DIR="${P2P_ROOT}/build"

BUILD_JOBS="${BUILD_JOBS:-8}"
TEST_BIN="${P2P_BUILD_DIR}/test/p2p-transfer_test"
...
echo "[INFO] Building bundled P2P-Transfer and test target..."
cmake -S "${P2P_ROOT}" -B "${P2P_BUILD_DIR}" -DCMAKE_BUILD_TYPE=Release
cmake --build "${P2P_BUILD_DIR}" --target p2p-transfer_test -j"${BUILD_JOBS}"
