...
Status InitAclAndDevice(int32_t deviceId)
{
    Status rc = testutil::EnsureAclInitialized();
    if (rc.IsError()) {
        return rc;
    }
...
Status RunOwner(const Options &opt)
{
    Status rc = InitAclAndDevice(opt.deviceId);
    if (rc.IsError()) {
        return rc;
    }
...
Status RunRequester(const Options &opt)
{
    Status rc = InitAclAndDevice(opt.deviceId);
    if (rc.IsError()) {
        return rc;
    ...
    Status readRc = engine.BatchTransferSyncRead(targetHostname, buffers, peerBufferAddresses, lengths);
    if (readRc.IsError()) {
...
Status RunRequesterConcurrent(const Options &opt)
{
    ...
    for (uint32_t i = 0; i < opt.requesterCount; ++i) {
        ...
        if (pid < 0) {
            return Status(StatusCode::kRuntimeError, "fork requester failed at index " + std::to_string(i));
        }
        if (pid == 0) {
            Options worker = opt;
    ...
    if (!allOk) {
        return Status(StatusCode::kRuntimeError, "concurrent requester failed: " + errStream.str());
    }
...
int main(int argc, char **argv)
{
    ...
    Status rc = (opt.role == "owner") ? RunOwner(opt) : RunRequesterConcurrent(opt);
    if (rc.IsError()) {
