...
public:
    ...
    Status QueryConnReady(const QueryConnReadyRequest &, QueryConnReadyResponse *rsp) override
    {
        if (rsp == nullptr) {
            return Status(StatusCode::kRuntimeError, "rsp is null");
        }
    ...
    Status ReadTrigger(const ReadTriggerRequest &, ReadTriggerResponse *rsp) override
    {
        if (rsp == nullptr) {
            return Status(StatusCode::kRuntimeError, "rsp is null");
        }
    ...
    Status BatchReadTrigger(const BatchReadTriggerRequest &, BatchReadTriggerResponse *rsp) override
    {
        if (rsp == nullptr) {
            return Status(StatusCode::kRuntimeError, "rsp is null");
        }
...
TEST(TransferControlDispatcherLltTest, InvalidPayloadReturnsInvalid)
{
    ...
    EXPECT_EQ(rsp.code, static_cast<int32_t>(StatusCode::kInvalid));
}
