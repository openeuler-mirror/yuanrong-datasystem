...
Status RpcCodeToStatus(int32_t code, const std::string &msg)
{
    if (code == kRpcOkCode) {
        return Status::OK();
    }
    switch (static_cast<StatusCode>(code)) {
        ...
        case StatusCode::kNotSupported:
            return TE_MAKE_STATUS(static_cast<StatusCode>(code), msg);
        default:
            return TE_MAKE_STATUS(StatusCode::kRuntimeError, msg);
    }
}

Status ParseTargetHostname(const std::string &targetHostname, std::string *peerHost, uint16_t *peerPort)
{
    TE_CHECK_PTR_OR_RETURN(peerHost);
    TE_CHECK_PTR_OR_RETURN(peerPort);
    TE_CHECK_OR_RETURN(!targetHostname.empty(), StatusCode::kInvalid, "targetHostname is empty");
    ...
    if (targetHostname.front() == '[') {
        ...
        TE_CHECK_OR_RETURN(close != std::string::npos, StatusCode::kInvalid, "invalid targetHostname");
        TE_CHECK_OR_RETURN(close + 2 <= targetHostname.size() && targetHostname[close + 1] == ':',
                           StatusCode::kInvalid, "targetHostname missing port");
        host = targetHostname.substr(1, close - 1);
        portStr = targetHostname.substr(close + 2);
    } else {
        const size_t sep = targetHostname.rfind(':');
        TE_CHECK_OR_RETURN(sep != std::string::npos, StatusCode::kInvalid, "targetHostname missing host:port");
        host = targetHostname.substr(0, sep);
        portStr = targetHostname.substr(sep + 1);
    }

    TE_CHECK_OR_RETURN(!host.empty(), StatusCode::kInvalid, "targetHostname host is empty");
    TE_CHECK_OR_RETURN(!portStr.empty(), StatusCode::kInvalid, "targetHostname port is empty");
    uint64_t portValue = 0;
    for (char c : portStr) {
        TE_CHECK_OR_RETURN(c >= '0' && c <= '9', StatusCode::kInvalid, "targetHostname port is invalid");
        portValue = portValue * 10 + static_cast<uint64_t>(c - '0');
        TE_CHECK_OR_RETURN(portValue <= 65535, StatusCode::kInvalid, "targetHostname port is invalid");
    }
    TE_CHECK_OR_RETURN(portValue > 0, StatusCode::kInvalid, "targetHostname port should be positive");
...
Status TransferEngine::Initialize(const std::string &localHostname, const std::string &protocol,
                                  const std::string &deviceName)
{
    ...
    TE_CHECK_OR_RETURN(!initialized_, StatusCode::kInvalid, "transfer engine already initialized");
    TE_CHECK_OR_RETURN(backend_ != nullptr, StatusCode::kInvalid, "backend is null");
    ...
    controlService_ = CreateTransferControlService(localHost_, localPort_, deviceId_, connMgr_, registeredMemory_,
                                                   backend_);
    Status startRc = controlServer_->Start(localHost_, localPort_, controlService_, rpcThreads_);
    if (startRc.IsError()) {
...
Status TransferEngine::RegisterMemory(uintptr_t bufferAddrRegisrterch, size_t length)
{
    return BatchRegisterMemory({bufferAddrRegisrterch}, {length});
}

Status TransferEngine::BatchRegisterMemory(const std::vector<uintptr_t> &bufferAddrs, const std::vector<size_t> &lengths)
{
    TE_CHECK_OR_RETURN(!bufferAddrs.empty(), StatusCode::kInvalid, "bufferAddrs is empty");
    TE_CHECK_OR_RETURN(bufferAddrs.size() == lengths.size(), StatusCode::kInvalid, "bufferAddrs/lengths size mismatch");

    std::lock_guard<std::mutex> lock(apiMutex_);
    TE_CHECK_OR_RETURN(initialized_, StatusCode::kNotReady, "transfer engine not initialized");
    TE_CHECK_OR_RETURN(deviceId_ >= 0, StatusCode::kNotReady, "device_id is invalid");

    std::vector<uint64_t> addedBaseAddrs;
    addedBaseAddrs.reserve(bufferAddrs.size());
    for (size_t i = 0; i < bufferAddrs.size(); ++i) {
        TE_CHECK_OR_RETURN(bufferAddrs[i] > 0, StatusCode::kInvalid, "bufferAddr should be positive");
        TE_CHECK_OR_RETURN(lengths[i] > 0, StatusCode::kInvalid, "length should be positive");
        ...
        if (!registeredMemory_->AddRegion(region)) {
            ...
            return Status(StatusCode::kInvalid, "failed to add registered region");
        }
...
Status TransferEngine::UnregisterMemory(uintptr_t bufferAddrRegisrterch)
{
    return BatchUnregisterMemory({bufferAddrRegisrterch});
}

Status TransferEngine::BatchUnregisterMemory(const std::vector<uintptr_t> &bufferAddrs)
{
    std::lock_guard<std::mutex> lock(apiMutex_);
    TE_CHECK_OR_RETURN(initialized_, StatusCode::kNotReady, "transfer engine not initialized");
    TE_CHECK_OR_RETURN(!bufferAddrs.empty(), StatusCode::kInvalid, "bufferAddrs is empty");
    for (size_t i = 0; i < bufferAddrs.size(); ++i) {
        ...
        TE_CHECK_OR_RETURN(registeredMemory_->RemoveByBaseAddr(static_cast<uint64_t>(bufferAddrs[i])),
                           StatusCode::kNotFound, "region is not registered");
    }
    LOG(INFO) << "batch unregister memory success, count=" << bufferAddrs.size();
    return Status::OK();
}

Status TransferEngine::TransferSyncRead(const std::string &targetHostname, uintptr_t buffer, uintptr_t peerBufferAddress,
                                        size_t length)
{
    return BatchTransferSyncRead(targetHostname, {buffer}, {peerBufferAddress}, {length});
}

Status TransferEngine::BatchTransferSyncRead(const std::string &targetHostname, const std::vector<uintptr_t> &buffers,
                                             const std::vector<uintptr_t> &peerBufferAddresses,
                                             const std::vector<size_t> &lengths)
{
    internal::DumpProcessEnvironment("batch_transfer_sync_read_begin");
    TE_CHECK_OR_RETURN(!buffers.empty(), StatusCode::kInvalid, "buffers is empty");
    TE_CHECK_OR_RETURN(buffers.size() == peerBufferAddresses.size() && buffers.size() == lengths.size(),
                       StatusCode::kInvalid, "buffers/peerBufferAddresses/lengths size mismatch");

    std::string peerHost;
    uint16_t peerPort = 0;
    TE_RETURN_IF_ERROR(ParseTargetHostname(targetHostname, &peerHost, &peerPort));
    for (size_t i = 0; i < buffers.size(); ++i) {
        TE_CHECK_OR_RETURN(buffers[i] > 0, StatusCode::kInvalid, "buffer should be positive");
        TE_CHECK_OR_RETURN(peerBufferAddresses[i] > 0, StatusCode::kInvalid, "peerBufferAddress should be positive");
        TE_CHECK_OR_RETURN(lengths[i] > 0, StatusCode::kInvalid, "length should be positive");
    }
    ...
    uint64_t requestIdStart = 0;
    {
        std::lock_guard<std::mutex> lock(apiMutex_);
        TE_CHECK_OR_RETURN(initialized_, StatusCode::kNotReady, "transfer engine not initialized");
        TE_CHECK_OR_RETURN(!finalizing_, StatusCode::kNotReady, "transfer engine is finalizing");
    ...
    Status connRc = BuildConnectionIfNeeded(peerHost, peerPort, &ownerDeviceId);
    if (connRc.IsError()) {
    ...
    for (size_t i = 0; i < buffers.size(); ++i) {
        Status postRecvRc = backend_->PostRecv(spec, static_cast<uint64_t>(buffers[i]), static_cast<uint64_t>(lengths[i]));
    ...
    Status triggerRc = controlClient_->BatchReadTrigger(peerHost, peerPort, req, &rsp);
    ...
    Status rpcStatus = RpcCodeToStatus(rsp.code, rsp.msg);
    ...
    for (size_t i = 0; i < buffers.size(); ++i) {
        Status waitRc = backend_->WaitRecv(spec, kDefaultRecvWaitTimeoutMs);
        if (waitRc.IsError()) {
...
Status TransferEngine::Finalize()
{
    {
...
Status TransferEngine::BuildConnectionIfNeeded(const std::string &peerHost, uint16_t peerPort, int32_t *ownerDeviceId)
{
    ...
    if (cachedOwnerDeviceId >= 0) {
        ...
        if (connMgr_->HasReadyConnection(key)) {
            ...
            Status queryRc = controlClient_->QueryConnReady(peerHost, peerPort, queryReq, &queryRsp);
            if (queryRc.IsOk()) {
                Status rpcStatus = RpcCodeToStatus(queryRsp.code, queryRsp.msg);
...
Status TransferEngine::BuildConnectionOnce(const std::string &peerHost, uint16_t peerPort, int32_t *ownerDeviceId)
{
    TE_CHECK_PTR_OR_RETURN(ownerDeviceId);
    ...
    std::string rootInfo;
    Status createRootRc = backend_->CreateRootInfo(&rootInfo);
    ...
    Status exchangeRc = controlClient_->ExchangeRootInfo(peerHost, peerPort, exchangeReq, &exchangeRsp);
    ...
    Status exchangeRpcStatus = RpcCodeToStatus(exchangeRsp.code, exchangeRsp.msg);
    ...
    TE_CHECK_OR_RETURN(exchangeRsp.ownerDeviceId >= 0, StatusCode::kRuntimeError, "owner_device_id is invalid");
    ...
    Status initRecvRc = backend_->InitRecv(spec, rootInfo);
