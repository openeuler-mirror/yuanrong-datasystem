...
public:
    ...
    Status ExchangeRootInfo(const ExchangeRootInfoRequest &req, ExchangeRootInfoResponse *rsp) override
    {
        internal::DumpProcessEnvironment("service_exchange_root_info");
        TE_CHECK_PTR_OR_RETURN(rsp);
        TE_CHECK_OR_RETURN(!req.requesterHost.empty(), StatusCode::kInvalid, "requester_host is empty");
        TE_CHECK_OR_RETURN(req.requesterPort > 0, StatusCode::kInvalid, "requester_port should be positive");
        TE_CHECK_OR_RETURN(req.requesterDeviceId >= 0, StatusCode::kInvalid, "requester_device_id should be non-negative");
        TE_CHECK_OR_RETURN(!req.rootInfo.empty(), StatusCode::kInvalid, "root_info is empty");
        ...
        task.rootInfo = req.rootInfo;
        {
            ...
            if (ownerInitQueue_.size() >= kMaxOwnerInitQueueSize) {
                rsp->code = static_cast<int32_t>(StatusCode::kNotReady);
    ...
    Status QueryConnReady(const QueryConnReadyRequest &req, QueryConnReadyResponse *rsp) override
    {
        TE_CHECK_PTR_OR_RETURN(rsp);
        TE_CHECK_OR_RETURN(!req.requesterHost.empty(), StatusCode::kInvalid, "requester_host is empty");
        TE_CHECK_OR_RETURN(req.requesterPort > 0, StatusCode::kInvalid, "requester_port should be positive");
    ...
    Status ReadTrigger(const ReadTriggerRequest &req, ReadTriggerResponse *rsp) override
    {
        internal::DumpProcessEnvironment("service_read_trigger");
        TE_CHECK_PTR_OR_RETURN(rsp);
        TE_CHECK_OR_RETURN(req.length > 0, StatusCode::kInvalid, "length should be positive");
        TE_CHECK_OR_RETURN(req.remoteAddr > 0, StatusCode::kInvalid, "remote_addr should be positive");
        int32_t registeredNpuId = -1;
        if (!registeredMemory_->FindDeviceIdByRange(req.remoteAddr, req.length, &registeredNpuId)) {
            rsp->code = static_cast<int32_t>(StatusCode::kNotAuthorized);
        ...
        if (backend_->RequiresAclRuntime()) {
            if (registeredNpuId != localDeviceId_) {
                rsp->code = static_cast<int32_t>(StatusCode::kNotSupported);
        ...
        Status sendRc = backend_->PostSend(spec, req.remoteAddr, req.length);
        if (sendRc.IsError()) {
    ...
    Status BatchReadTrigger(const BatchReadTriggerRequest &req, BatchReadTriggerResponse *rsp) override
    {
        ...
        TE_CHECK_OR_RETURN(req.requesterPort > 0, StatusCode::kInvalid, "requester_port should be positive");
        TE_CHECK_OR_RETURN(req.requesterDeviceId >= 0, StatusCode::kInvalid, "requester_device_id should be non-negative");
        TE_CHECK_OR_RETURN(!req.items.empty(), StatusCode::kInvalid, "batch items is empty");
        ...
        for (size_t i = 0; i < req.items.size(); ++i) {
            ...
            Status singleRc = ReadTrigger(singleReq, &singleRsp);
            if (singleRc.IsError()) {
