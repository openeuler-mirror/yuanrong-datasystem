...
Status DispatchControlRequest(const std::shared_ptr<ITransferControlService> &service, RpcMethod method,
                              ...
                              std::vector<uint8_t> *rspPayload)
{
    ...
    TE_CHECK_OR_RETURN(service != nullptr, StatusCode::kRuntimeError, "service is null");
    ...
    *rspMethod = method;
    ...
    if (method == RpcMethod::kQueryConnReady) {
        ...
        if (!DecodeQueryReq(reqPayload, &req)) {
            ...
            Status rc = service->QueryConnReady(req, &rsp);
            if (rc.IsError()) {
                rsp.code = static_cast<int32_t>(rc.GetCode());
                rsp.msg = rc.GetMsg();
                rsp.ready = false;
    ...
    if (method == RpcMethod::kReadTrigger) {
        ...
        if (!DecodeReadReq(reqPayload, &req)) {
            ...
            Status rc = service->ReadTrigger(req, &rsp);
            if (rc.IsError()) {
                rsp.code = static_cast<int32_t>(rc.GetCode());
                rsp.msg = rc.GetMsg();
    ...
    if (method == RpcMethod::kBatchReadTrigger) {
        ...
        if (!DecodeBatchReadReq(reqPayload, &req)) {
            ...
            Status rc = service->BatchReadTrigger(req, &rsp);
            if (rc.IsError()) {
                rsp.code = static_cast<int32_t>(rc.GetCode());
                rsp.msg = rc.GetMsg();
                rsp.failedItemIndex = -1;
