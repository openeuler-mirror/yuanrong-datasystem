...
public:
    ...
    /**
     * @brief Build the cluster-table key used by IClusterStore for one worker.
...
public:
    ...
    /**
     * @brief Decode one cluster-store event into a typed membership event.
     * @param[in] event Raw cluster-store event.
     ...
     */
    virtual Status HandleWorkerEvent(const ClusterStoreEvent &event, WorkerWatchEvent &typed) = 0;
};

class ClusterRegistry final : public IClusterRegistry {
public:
    explicit ClusterRegistry(IClusterStore &store);
