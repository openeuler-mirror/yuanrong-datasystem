...
public:
    ...
    /**
     * @brief Decode one committed-topology event delivered by the shared IClusterStore event dispatcher.
     ...
     */
    virtual Status HandleCommittedTopologyEvent(const ClusterStoreEvent &event, TopologyWatchEvent &typed) = 0;
};

class TopologyRepository final : public ITopologyRepository {
public:
    explicit TopologyRepository(IClusterStore &store);
    ...
    Status HandleCommittedTopologyEvent(const ClusterStoreEvent &event, TopologyWatchEvent &typed) override;
