...
std::string RpcGenerator::BuildCallMethodPrologue()
{
    return
        ...
        "    // Strip the per-request traceID + log sampling state prefix from the\n"
        "    // request attachment, then restore both into the per-bthread Trace so\n"
        "    // the generated handler participates in LogSampler instead of being\n"
        "    // bypassed. Wire format and defensive defaulting live in\n"
        "    // ExtractTraceIDAndSampleState(); ApplyLogSampleState() restores\n"
        "    // requestLogTrace/decision on the same Trace ScopedRequestContext set.\n"
        "    std::string rpcTraceID;\n"
        "    ::datasystem::LogSampleState rpcSampleState = ::datasystem::LOG_SAMPLE_UNDECIDED;\n"
        "    ::datasystem::ExtractTraceIDAndSampleState(cntl->request_attachment(), rpcTraceID, rpcSampleState);\n"
        "    ::datasystem::ScopedRequestContext scopedCtx(rpcTraceID);\n"
        "    ::datasystem::ApplyLogSampleState(rpcSampleState);\n"
        "    switch (method->index()) {\n";
}
                                                ...
                                                const std::string &indent)
{
    ...
    std::string impl =
        ...
        "$indent$SLOW_LOG_IF_OR_VLOG(\n"
        "$indent$    INFO, FLAGS_enable_perf_trace_log, 1,\n"
        "$indent$    FormatString(\"ServerRecv ts %llu tid %d cpu %d bid %llu\\n\", rpcTrace.ServerRecvTs(),\n"
        "$indent$                 BrpcTraceGetTid(), sched_getcpu(),\n"
        "$indent$                 static_cast<unsigned long long>(bthread_self())));\n"
        "$indent$";
    ...
    impl +=
        ...
        "$indent$SLOW_LOG_IF_OR_VLOG(\n"
        "$indent$    INFO, FLAGS_enable_perf_trace_log, 1,\n"
        "$indent$    FormatString(\"ServerSend ts %llu tid %d cpu %d bid %llu\\n\", rpcTrace.ServerSendTs(),\n"
        "$indent$                 BrpcTraceGetTid(), sched_getcpu(),\n"
        "$indent$                 static_cast<unsigned long long>(bthread_self())));\n"
                                                      ...
                                                      const std::string &indent)
{
    ...
    std::string impl =
        ...
        "$indent$    method->full_name());\n"
        "$indent$rpcTrace.MarkServerRecv();\n"
        "$indent$VLOG(1) << FormatString(\"ServerRecv ts %llu tid %d cpu %d bid %llu\\n\", rpcTrace.ServerRecvTs(), BrpcTraceGetTid(), sched_getcpu(), static_cast<unsigned long long>(bthread_self()));\n"
        "$indent$";
    impl += BuildScTimeoutDurationInitSnippet();
    impl +=
        "$indent$rpcTrace.MarkServerExecStart();\n"
        ...
        "$indent$}\n"
        "$indent$rpcTrace.MarkServerSend();\n"
        "$indent$VLOG(1) << FormatString(\"ServerSend ts %llu tid %d cpu %d bid %llu\\n\", rpcTrace.ServerSendTs(), BrpcTraceGetTid(), sched_getcpu(), static_cast<unsigned long long>(bthread_self()));\n"
        "$indent$::datasystem::AppendBrpcServerTraceTrailer(rpcTrace, cntl->response_attachment());\n";
    printer.Print(vars, impl.c_str());
}
                                                      ...
                                                      const std::string &indent)
{
    ...
    impl +=
        ...
        "$indent$::datasystem::GetRequestContext()->reqTimeoutDuration = serverApi->GetScTimeoutDuration();\n"
        "$indent$::datasystem::GetRequestContext()->scTimeoutDuration = serverApi->GetScTimeoutDuration();\n"
...
std::string RpcGenerator::BuildScTimeoutDurationInitSnippet()
{
    ...
    return
        ...
        "$indent$        int64_t remainingUs = deadlineUs - nowUs;\n"
        ...
        "$indent$    }\n"
        "$indent$    if (deadlineUs > 0 && remainingMs <= 0) {\n"
        "$indent$        // Keep an expired absolute deadline expired.\n"
        "$indent$    } else if (remainingMs > 0) {\n"
        "$indent$        ::datasystem::GetRequestContext()->reqTimeoutDuration.Init(remainingMs);\n"
        ...
        "$indent$        }\n"
        "$indent$    } else {\n"
        "$indent$        ::datasystem::GetRequestContext()->reqTimeoutDuration.Init();\n"
                                                       ...
                                                       const std::string &indent)
{
    ...
    impl +=
        ...
        "$indent$::datasystem::GetRequestContext()->scTimeoutDuration = pimpl->GetScTimeoutDuration();\n"
        "$indent$auto reader =\n"
                                                     ...
                                                     const std::string &indent)
{
    ...
    std::string impl =
        ...
        "$indent$::datasystem::GetRequestContext()->scTimeoutDuration = brpcStreamImpl->GetScTimeoutDuration();\n"
        "$indent$auto stream =\n"
