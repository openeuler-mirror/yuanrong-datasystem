...
Status Evaluate(const WorkerRuntimeStateSnapshot &snapshot, WorkerAdmissionKind kind, const std::string &operation)
{
    switch (snapshot.mode) {
        ...
        case WorkerServiceMode::OUT_OF_MEMORY:
            if (kind == WorkerAdmissionKind::NORMAL_READ || kind == WorkerAdmissionKind::INTERNAL_JOINING_RPC
...
WorkerServiceAdmission::WorkerServiceAdmission(const WorkerRuntimeState &runtimeState) : runtimeState_(runtimeState)
{
}
