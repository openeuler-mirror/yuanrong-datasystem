...
DS_DEFINE_uint32(eviction_thread_num, 1,
                 "Deprecated. Ignored; memory eviction uses a fixed single thread.");
...
void WorkerOcEvictionManager::EvictionTraceAggregator::Add(const EvictionTrace &trace)
{
    ...
    if (elapsed > 1 || trace.rc.IsError()) {
        ...
        auto logLevel = trace.rc.IsError() ? 0 : 1;
    ...
    if (trace.rc.IsError()) {
        if (!trace.objectKeySizeMap.empty()) {
            for (const auto &item : trace.objectKeySizeMap) {
                summary.failedKeys.emplace_back(item.first);
            }
        } else {
        ...
        if (!trace.objectKeySizeMap.empty()) {
            for (const auto &item : trace.objectKeySizeMap) {
                summary.successKeys.emplace_back(item.first);
            }
        } else {
