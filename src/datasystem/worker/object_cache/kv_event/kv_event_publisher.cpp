...
struct KvEventPublisher::QueueState {
    std::mutex mutex;
...
KvEventPublisher::KvEventPublisher(KvEventConfig config)
    : config_(std::move(config)), queueState_(std::make_unique<QueueState>())
{
    Initialize();
}
...
void KvEventPublisher::Enqueue(PendingEvent event)
{
    ...
    {
        std::lock_guard<std::mutex> lock(queueState_->mutex);
        if (queueState_->queue.size() >= config_.queueCapacity) {
            droppedCount = droppedEvents_.fetch_add(1, std::memory_order_relaxed) + 1;
        } else {
            queueState_->queue.push_back(std::move(event));
        }
    }
    if (droppedCount != 0) {
...
std::vector<KvEventPublisher::PendingEvent> KvEventPublisher::TakeBatch(bool &shouldExit)
{
    ...
    std::unique_lock<std::mutex> lock(queueState_->mutex);
    while (!stop_.load(std::memory_order_acquire) && queueState_->queue.empty()) {
        queueState_->cv.wait(lock);
    }
    if (queueState_->queue.empty() && stop_.load(std::memory_order_acquire)) {
        shouldExit = true;
        return batch;
    }
    INJECT_POINT_NO_RETURN("KvEventPublisher.TakeBatch.beforeMove", [] { throw std::bad_alloc(); });
    while (!queueState_->queue.empty() && batch.size() < K_MAX_BATCH_SIZE) {
        batch.push_back(std::move(queueState_->queue.front()));
        queueState_->queue.pop_front();
    }
    return batch;
}
