 ...
 * Mirrors ShmReceiveBufferOwner but holds a TransportRequestContext (write side) and gates
 * Buffer::Publish on session liveness via CheckAlive (K_BUFFER_DEPRECATED), so a Publish whose
 * target session was torn down between Create and Publish does not write into a stale region. */
...
/** @brief Send-buffer owner that releases the target worker reference through the ShmSession
 *  for routed Create-allocated shared-memory write buffers. */
class ShmSendBufferOwner final : public IReceiveBufferOwner {
public:
    ShmSendBufferOwner(std::shared_ptr<ShmSession> session, std::shared_ptr<IMmapTableEntry> mmapEntry,
                       ShmKey shmId, TransportRequestContext context, std::weak_ptr<ThreadPool> releasePool);
