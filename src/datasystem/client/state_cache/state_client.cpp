...
StateClient::StateClient(const ConnectOptions &connectOptions)
{
...
Status StateClient::Set(const std::string &key, const StringView &val, const SetParam &setParam)
{
   TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
...
std::string StateClient::Set(const StringView &val, const SetParam &setParam)
{
   TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
   PerfPoint point(PerfKey::KV_CLIENT_SET_OBJECT);
   AccessRecorder accessPoint(AccessRecorderKey::DS_KV_CLIENT_SET);
...
Status StateClient::MSetTx(const std::vector<std::string> &keys, const std::vector<StringView> &vals,
...
Status StateClient::Get(const std::string &key, std::string &val, int32_t timeoutMs)
{
   TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
...
Status StateClient::Get(const std::vector<std::string> &keys, std::vector<std::string> &vals, int32_t timeoutMs)
{
   TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
   ...
   size_t dataSize = 0;
   AccessRecorder accessPoint(AccessRecorderKey::DS_KV_CLIENT_GET);
   Status rc = impl_->GetWithLatch(keys, vals, timeoutMs, buffers, dataSize);
   RequestParam reqParam;
...
Status StateClient::Get(const std::string &key, Optional<ReadOnlyBuffer> &readOnlyBuffer, int32_t timeoutMs)
{
   TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
   PerfPoint point(PerfKey::KV_CLIENT_GET_BUFFER);
   std::vector<Optional<Buffer>> buffers;
   AccessRecorder accessPoint(AccessRecorderKey::DS_KV_CLIENT_GET);
   Status rc = impl_->Get({ key }, timeoutMs, buffers);
...
Status StateClient::MSet(const std::vector<std::string> &keys, const std::vector<StringView> &vals,
...
Status StateClient::Get(const std::vector<std::string> &keys, std::vector<Optional<ReadOnlyBuffer>> &readOnlyBuffers,
                       int32_t timeoutMs)
{
   TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
   ...
   std::vector<Optional<Buffer>> buffers;
   AccessRecorder accessPoint(AccessRecorderKey::DS_KV_CLIENT_GET);
   Status rc = impl_->Get(keys, timeoutMs, buffers);
   int64_t dataSize = 0;
   if (rc.IsOk()) {
...
static std::string ReadParamToString(const std::vector<ReadParam> &params)
{
   ...
   for (const auto &param : params) {
       ...
       if (len > LOG_TOTAL_KEYS_SIZE_LIMIT - size) {
...
Status StateClient::Read(const std::vector<ReadParam> &readParams,
...
Status StateClient::Del(const std::string &key)
{
   TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
...
Status StateClient::Del(const std::vector<std::string> &keys, std::vector<std::string> &failedKeys)
{
   TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
...
std::string StateClient::GenerateKey(const std::string &prefixKey)
{
   TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
   std::string key;
...
Status StateClient::QuerySize(const std::vector<std::string> &objectKeys, std::vector<uint64_t> &outSizes)
{
   return impl_->QuerySize(objectKeys, outSizes);
}

Status StateClient::HealthCheck()
{
    ServerState state;
    return impl_->HealthCheck(state);
}

Status StateClient::Exist(const std::vector<std::string> &keys, std::vector<bool> &exists)
{
    TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
    return impl_->Exist(keys, exists, true, false);
}

Status StateClient::Expire(const std::vector<std::string> &keys, uint32_t ttlSeconds,
                           std::vector<std::string> &failedKeys)
{
   TraceGuard traceGuard = Trace::Instance().SetTraceUUID();
