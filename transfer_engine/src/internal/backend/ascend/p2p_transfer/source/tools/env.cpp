...
bool TryMapVisibleToLogicDeviceId(uint32_t visibleOrPhyDevId, uint32_t &logicDeviceId)
{
    ...
    if (visibleEnv == nullptr || visibleEnv[0] == '\0') {
        p2p::LogInfo("TryMapVisibleToLogicDeviceId: ASCEND_RT_VISIBLE_DEVICES unset, skip mapping");
        return false;
    }

    p2p::LogInfo(std::string("TryMapVisibleToLogicDeviceId: input=") + std::to_string(visibleOrPhyDevId) +
                 ", env=" + visibleEnv);
    ...
    while (std::getline(ss, token, ',')) {
        ...
        if (static_cast<uint32_t>(val) == visibleOrPhyDevId) {
            logicDeviceId = idx;
            p2p::LogInfo(std::string("TryMapVisibleToLogicDeviceId: matched token=") + std::to_string(val) +
                         ", logic_index=" + std::to_string(logicDeviceId));
            return true;
        }
        ++idx;
    }
    p2p::LogWarning(std::string("TryMapVisibleToLogicDeviceId: no match for input=") +
                    std::to_string(visibleOrPhyDevId));
    return false;
}
