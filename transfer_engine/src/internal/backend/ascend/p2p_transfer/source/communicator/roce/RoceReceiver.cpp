...
void SetReceiverIp(ReceiverData &receiverData, const P2PIpAddress &ipAddr)
{
    receiverData.set_recvnpuipfamily(ipAddr.family);
    receiverData.set_recvnpuip(EncodeIpBytes(ipAddr));
    if (ipAddr.family == AF_INET) {
        receiverData.set_recvnpuipv4(in_addr_to_uint32(ipAddr.addr.addr));
    }
}
...
{
    uint32_t logicDevIndex = static_cast<uint32_t>(recvDeviceId);
    uint32_t phyId = 0;
    ...
    p2p::LogInfo(std::string("RoceReceiver::Initialize phy query #1, recv_device_id=") +
                 std::to_string(recvDeviceId) + ", logic_device_id=" + std::to_string(logicDevIndex) +
                 ", rc=" + std::to_string(static_cast<int>(phyRc)));
    if (phyRc != RT_ERROR_NONE) {
        if (!TryMapVisibleToLogicDeviceId(static_cast<uint32_t>(recvDeviceId), logicDevIndex)) {
            ACL_CHECK_STATUS(rtGetDeviceIndexByPhyId(static_cast<uint32_t>(recvDeviceId), &logicDevIndex));
            p2p::LogInfo(std::string("RoceReceiver::Initialize fallback rtGetDeviceIndexByPhyId success, input=") +
                         std::to_string(recvDeviceId) + ", logic_device_id=" + std::to_string(logicDevIndex));
        } else {
            p2p::LogInfo(std::string("RoceReceiver::Initialize fallback env map success, input=") +
                         std::to_string(recvDeviceId) + ", logic_device_id=" + std::to_string(logicDevIndex));
        }
        ACL_CHECK_STATUS(rtGetDevicePhyIdByIndex(logicDevIndex, &phyId));
        p2p::LogInfo(std::string("RoceReceiver::Initialize phy query #2 success, logic_device_id=") +
                     std::to_string(logicDevIndex) + ", phy_id=" + std::to_string(phyId));
    } else {
        p2p::LogInfo(std::string("RoceReceiver::Initialize phy query #1 success, phy_id=") + std::to_string(phyId));
    }
    ...
    rdmaSocket = std::make_unique<RdmaSocket>(phyId, localIp, SERVER);
    ...
    CHECK_STATUS(DecodeIpAddress(senderData.sendnpuipfamily(), senderData.sendnpuip(), senderData.sendnpuipv4(),
                                 remoteIp));
    ...
    SetReceiverIp(receiverData, localIp);
    ...
    p2p::LogInfo(std::string("RoceReceiver::Initialize wait RoCE socket, local_npu_ip=") +
                 IpAddressToString(localIp) + ", listen_port=" + std::to_string(listenPort) +
                 ", remote_npu_ip=" + IpAddressToString(remoteIp) + ", timeout_disabled=true");
