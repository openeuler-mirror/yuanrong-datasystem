...
Status P2PCommunicator::StartClient(P2PRootHandle &rootHandle)
{
    ...
    p2p::LogInfo("P2PCommunicator::StartClient success");
    return Status::Success();
}
...
{
    p2p::LogInfo(std::string("P2PCommunicator::EstablishConnection begin, is_root=") + (isRoot ? "true" : "false"));
    if (isRoot) {
    ...
    p2p::LogInfo(std::string("P2PCommunicator::EstablishConnection choose channel=") +
                 std::to_string(static_cast<int>(channelType)) + ", role=" +
                 std::to_string(static_cast<int>(args.role)) + ", client_ip=" + clientIp + ", server_ip=" + serverIp);
    ...
    established = true;
    p2p::LogInfo("P2PCommunicator::EstablishConnection success");
...
Status P2PCommunicator::CreateServer()
{
    ...
    p2p::LogInfo(std::string("P2PCommunicator::CreateServer port_range=") + std::to_string(startPort) + "-" +
...
{
    p2p::LogInfo("P2PCommunicator::AcceptClient begin");
    if (!server) {
    ...
    CHECK_STATUS(server->SendObject(authResponse));
    p2p::LogInfo(std::string("P2PCommunicator::AcceptClient success, client_ip=") + clientIp);
    return Status::Success();
}

Status P2PCommunicator::CreateClient(std::string ip, uint16_t port)
{
    p2p::LogInfo(std::string("P2PCommunicator::CreateClient begin, ip=") + ip + ", port=" + std::to_string(port));
    ...
    p2p::LogInfo("P2PCommunicator::CreateClient success");
...
{
    p2p::LogInfo("P2PCommunicator::ConnectServer begin");
    ...
    p2p::LogInfo("P2PCommunicator::ConnectServer success");
    return Status::Success();
}
