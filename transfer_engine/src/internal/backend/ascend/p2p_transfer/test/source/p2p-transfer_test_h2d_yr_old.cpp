...
int SendDeviceLogic(const char *recvSendFifoPath, uint32_t sendDeviceId, size_t bufferSizeBytes, uint32_t recvRank,
                    uint32_t sendRank, int nSamples, int batchSize, int keyNum, pthread_barrier_t *barrier,
                    int nDevices)
{
    ...
    float **hostBuffs = (float **)malloc(keyNum * sizeof(float *));
    for (int k = 0; k < keyNum; k++) {
        hostBuffs[k] = (float *)mmap(nullptr, batchSize * bufferSizeBytes, PROT_READ | PROT_WRITE,
                                     MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB, -1, 0);
        if (hostBuffs[k] == MAP_FAILED) {
            std::cerr << "Mmap failed" << std::endl;
            exit(1);
        }
        memset_s(hostBuffs[k], batchSize * bufferSizeBytes, 0, batchSize * bufferSizeBytes);
    }
    ...
    for (int k = 0; k < keyNum; k++) {
        for (int i = 0; i < (batchSize * bufferSizeBytes) / sizeof(float); i++) {
            hostBuffs[k][i] = static_cast<float>(i);
        }
    }
    ...
    for (int k = 0; k < keyNum; k++) {
        ...
        NPU_ERROR(
            P2PRegisterHostMem(hostBuffs[k], batchSize * bufferSizeBytes, &hostSegmentInfo, P2P_SEGMENT_READ_ONLY));
        write(recvSendFd, &hostSegmentInfo, sizeof(P2pSegmentInfo));  // Send message to FIFO
        write(recvSendFd, &hostBuffs[k], sizeof(float *));            // Send message to FIFO
    }
                    ...
                    int nDevices)
{
    ...
    float **remoteHostBuffs =
        (float **)malloc(keyNum * sizeof(float *));  // might want to allocate multiple hostBuffs, for all
    for (int k = 0; k < keyNum; k++) {
        P2pSegmentInfo remoteHostSegmentInfo;
        read(recvSendFd, &remoteHostSegmentInfo, sizeof(P2pSegmentInfo));  // Read from fifo
        read(recvSendFd, &remoteHostBuffs[k], sizeof(float *));            // Send message to FIFO
    ...
    for (int k = 0; k < keyNum; k++) {
        for (int i = 0; i < batchSize; i++) {
            NPU_ERROR(P2PGetRemoteHostMem(&remoteHostBuffs[k][i * (bufferSizeBytes / sizeof(float))], dRecvBuffs[i],
    ...
    for (int sample = 0; sample < nSamples; sample++) {
        for (int k = 0; k < keyNum; k++) {
            ...
            for (int i = 0; i < batchSize; i++) {
                transferSizes[i] = bufferSizeBytes / sizeof(float);
            }
#endif

#ifdef USE_BATCH
            NPU_ERROR(P2PScatterFromRemoteHostMem((void *)remoteHostBuffs[k], dRecvBuffs, transferSizes,
            ...
            for (int i = 0; i < batchSize; i++) {
                NPU_ERROR(aclrtMemcpy(&verifyBuffs[k][i * (bufferSizeBytes / sizeof(float))], bufferSizeBytes,
                                      dRecvBuffs[i], bufferSizeBytes, ACL_MEMCPY_DEVICE_TO_HOST));
                NPU_ERROR(aclrtMemset(dRecvBuffs[i], bufferSizeBytes, 0, bufferSizeBytes));
            }

            Verify(verifyBuffs[k], batchSize * bufferSizeBytes / sizeof(float), 0);
            std::cout << "Verified " << k << std::endl;
#endif

            double bw = (batchSize * bufferSizeBytes) / time.Value();
...
int Benchmark()
{
    ...
    uint32_t nDevices = 2;
    size_t bufferSizeBytes = static_cast<size_t>(312);
    ...
    if (pid == 0) {  // Child
        ...
        SendDeviceLogic(recvSendFifoPath, sendDeviceId, bufferSizeBytes, 0, 1, nSamples, batchSize, keyNum, barrier,
                        nDevices);

        return 0;
    } else {  // Parent
        std::cout << "Process " << pid << std::endl;
        RecvDeviceLogic(recvSendFifoPath, recvDeviceId, bufferSizeBytes, 0, 1, nSamples, batchSize, keyNum, barrier,
                        nDevices);
