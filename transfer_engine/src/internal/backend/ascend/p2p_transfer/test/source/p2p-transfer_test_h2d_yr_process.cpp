    ...
    for (int sample = 0; sample < nSamples; sample++) {
        for (int i = 0; i < numObjChoices.size(); i++) {
            ...
            for (int k = 0; k < keyNum; k++) {
                ...
                NPU_ERROR(P2PScatterFromRemoteHostMem((void *)srcBuf, dRecvBuffs[k], transferSizes, HCCL_DATA_TYPE_FP32,
                                                      batchSize, p2pComm, stream));
#else
                for (int i = 0; i < batchSize; i++) {
                    // Note: Wrong
                    NPU_ERROR(P2PGetRemoteHostMem(&remoteHostBuff[i * (bufferSizeBytes / sizeof(float))],
                                                  dRecvBuffs[k][i], bufferSizeBytes / sizeof(float),
                                                  HCCL_DATA_TYPE_FP32, p2pComm, stream));
                }
#endif
            }
            NPU_ERROR(aclrtSynchronizeStream(stream));
...
int Benchmark()
{
    ...
    if (pid == 0) {  // Child
        ...
        SendDeviceLogic(recvSendFifoPath, sendDeviceId, bufferSizeBytes, 0, 1, nSamples, blksPerObj, maxKeyNum, barrier,
                        nDevices);

        return 0;
    } else {  // Parent
        std::cout << "Process " << pid << std::endl;
        RecvDeviceLogic(recvSendFifoPath, recvDeviceId, bufferSizeBytes, 0, 1, nSamples, blksPerObj, maxKeyNum, barrier,
                        nDevices);
