...
static int Write()
{
    ...
    std::cout << "State client set succeeds." << std::endl;
    return SUCCESS;
}
...
static int Read()
{
    ...
    if (correctVal == val) {
        std::cout << "State client get string value succeeds." << std::endl;
    } else {
    ...
    std::cout << "State client get succeeds." << std::endl;
    return SUCCESS;
}
...
static int Delete()
{
    ...
    std::cout << "State client delete succeeds." << std::endl;
    return SUCCESS;
}
...
static int TestSetValue()
{
    ...
    std::cout << "State client set value api succeeds." << std::endl;
    return SUCCESS;
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 1) {
        ...
        // example call:
        // ./state_example 127.0.0.1 18482 <client public key> <client private key> <worker public key>
        ip = argv[++index];
        port = atoi(argv[++index]);
    ...
    client_ = std::make_shared<StateClient>(connectOpts);
    (void)Context::SetTraceId("init");
    Status status = client_->Init();
    if (status.IsError()) {
        std::cerr << "Failed to init state client, detail: " << status.ToString() << std::endl;
        return FAILED;
    }

    if (Start() == FAILED) {
        std::cerr << "The state client example run failed." << std::endl;
        return FAILED;
    }
