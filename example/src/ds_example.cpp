...
static int Write(std::shared_ptr<ObjectClient> client, std::string writeId, std::string data, bool isSeal)
{
...
static int Read(std::shared_ptr<ObjectClient> client, std::string verifyId, std::string verifyData)
{
    std::cout << "Reading data from a buffer" << std::endl;
    std::vector<std::string> objKeys = { verifyId };
    int64_t timeout = 60;
    std::vector<Optional<Buffer>> buffers;
    Status status = client->Get(objKeys, timeout, buffers);
    if (!status.IsOk()) {
...
static int Modify(std::shared_ptr<ObjectClient> client, std::string writeId, std::string updateValue)
{
    ...
    std::cout << "Get: " << status.ToString() << std::endl;
    auto &buf = buffers[0];
    int newDataSize = updateValue.size();
    buf->WLatch();
...
int RunObjectTest(std::shared_ptr<ObjectClient> client)
{
...
int RunKVTest(std::shared_ptr<KVClient> kvClient)
{
