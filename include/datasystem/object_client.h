...
public:
    ...
    ///
    /// \param[in] key A non-empty object key with a valid object-key format.
    /// \param[out] prefix The object key (same as input key since worker UUID is no longer embedded).
    ///
    /// \return K_OK on success; K_INVALID when key is empty or invalid.
