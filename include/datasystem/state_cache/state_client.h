 ...
 */
#ifndef DATASYSTEM_STATE_CACHE_STATE_CLIENT_H
#define DATASYSTEM_STATE_CACHE_STATE_CLIENT_H
...
namespace datasystem {
class __attribute((visibility("default"))) StateClient {
public:
    /**
     * @brief Construct StateClient.
     * @param[in] connectOptions The connection options.
     */
    explicit StateClient(const ConnectOptions &connectOptions = {});
     ...
     * @param[out] outFailedKeys The failed ids for set
     * @param[in] param The set parameters.
     ...
     * @param[in] timeoutMs timeoutMs of waiting for the result return if object not ready. A positive integer number
     ...
     */
    Status Get(const std::string &key, std::string &val, int32_t timeoutMs = 0);
     ...
     *         K_RUNTIME_ERROR: Cannot get value from worker.
     */
    Status Get(const std::string &key, Optional<ReadOnlyBuffer> &readOnlyBuffer, int32_t timeoutMs = 0);
     ...
     * @endverbatim
     */
    Status Get(const std::vector<std::string> &keys, std::vector<std::string> &vals, int32_t timeoutMs = 0);
...
#endif  // DATASYSTEM_STATE_CACHE_STATE_CLIENT_H
