...
| `transfer_engine` | separate but related transfer subsystem | linked from root build when hetero and NPU-related flags are enabled; production data movement uses the CMake `-X on` auto-enabled D2D-over-HIXL backend, with a mock backend retained for tests |
