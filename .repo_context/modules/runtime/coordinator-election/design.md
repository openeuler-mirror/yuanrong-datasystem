...
After braft successfully initializes a non-empty `BootstrapPlan`, the Node publishes lifecycle `STARTED` without synthesizing the normalized `initial_conf` as committed configuration index 0. Only braft configuration callbacks update `CoordinatorRaftNode` and Manager committed-membership snapshots. During staggered fresh bootstrap, a peer that has already initialized braft can therefore report `VALID` metadata with no committed peers. When no observation contains a real non-empty committed configuration, that peer remains bootstrappable only if its candidate count/digest and immutable group/target identity match the current candidate view; otherwise it blocks bootstrap like any other identity mismatch.
...
| `CoordinatorRuntimeElectionTest` | Real Runtimes use empty config paths with stable fixture-owned common flags and per-generation endpoint/data/timing snapshots; covers target-majority bootstrap, deterministic over-target selection, membership replacement, Discovery failures, observed-Leader serving readiness, persisted restart, bounded quorum-loss stepdown, teardown, and port release. |
