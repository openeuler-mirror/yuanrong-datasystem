...
- 不通过 `eviction_thread_num` 暴露 memory eviction 并发调优；该 flag 仅作为 deprecated/ignored 兼容参数保留，主 eviction 并发仍由 `isDone_` 单任务门闩约束。
...
| `async_delete` | `DeleteAllCopyMetaReqPb` 请求字段；表示 master 侧删除语义可入队处理，但 worker RPC 仍同步等待响应 | `master_object.proto`, `OCMetadataManager::DeleteAllCopyMetaImpl` |

## Current State And Design Choice Notes

- Current implementation or baseline behavior:
  - `MemEvictionThread` 固定为源码内部单线程；`eviction_thread_num` 仅作为 deprecated/ignored 兼容参数保留。
  ...
  - `EvictSpilledObjects` 和 `SpillImpl` no-space fallback 仍同步调用 `DeleteNoneL2CacheEvictableObject`；该路径构造 `DeleteAllCopyMetaReqPb` 时使用 `ids_with_version`、本地 worker address、`redirect=true` 和 `async_delete=true`。
  - `eviction_thread_num` 保留为废弃兼容参数，且 dscli 默认配置、k8s deployment、k8s daemonset Helm values/template、中文部署文档和示例需要同步标注 deprecated/ignored。
...
- Known tradeoffs:
  ...
  - 单任务门闩让单纯增加 `MemEvictionThread` 数量在当前主流程中基本没有吞吐收益，因此 `eviction_thread_num` 虽保留兼容但运行时忽略。
...
5. 该同步 fallback 请求携带 `ids_with_version`、本地 worker address、`redirect=true` 和
   `async_delete=true`；primary end-life lane 也使用 `ids_with_version`。
6. master 端 `DeleteAllCopyMetaImpl` 读取 key/version 后放入 `DeleteObjectMediator`。
7. fallback RPC 仍同步等待 master 响应；`async_delete=true` 只描述 master 侧删除语义，不让 worker 调用异步返回。
...
- Why `eviction_thread_num` is not enough:
  ...
  - 该 flag 容易误导运维以为调大可以提升 eviction 并发；当前仅作为 deprecated/ignored 兼容参数保留，必须在源码、测试、dscli、k8s deployment、k8s daemonset、部署文档和示例中同步标注语义。
- Selected formal plan: worker-side primary end-life lane.
  ...
  - `DeleteAllCopyMeta` batch 使用 repeated `ids_with_version` 并设置 `async_delete=true`，并按 `outdated_objs`、`failed_object_keys`、
...
- Candidate 4: redesign true memory eviction concurrency.
  ...
  - `eviction_thread_num` 已废弃且运行时忽略；未来若重新设计并发，不能只重新启用该 flag，还要完整设计并发所有权和部署兼容语义。
...
- Common misleading symptoms:
  ...
  - `eviction_thread_num` 已废弃且运行时忽略；memory eviction 是否并发取决于 `isDone_` 和候选队列模型，而不是该兼容参数。
