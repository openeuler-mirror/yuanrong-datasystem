...
| Sanitizer | address: `-fsanitize=address -fno-omit-frame-pointer -g3`; thread: `-fsanitize=thread -g3`; undefined: `-fsanitize=undefined -fno-sanitize=alignment -fsanitize=float-cast-overflow -fno-sanitize-recover=all -O0` plus `-fno-sanitize=function` only when the compiler recognizes the `function` sub-check (guarded by `check_cxx_compiler_flag`); see `.bazelrc build:ubsan` for the Bazel-side UBSan-on-external-repos workaround | `CMakeLists.txt` |
