...
Host selection is opt-in through `--host-filter` JSON (`include`/`exclude` arrays) and repeatable `--host-ip`/`--exclude-host-ip`. JSON and CLI lists merge with exclusion priority; selection precedes prefixes and batching. Kubernetes Client selection reads live HostIP once per namespace; excluded targets receive no remote collection calls. Invalid selectors or unavailable target host IP fail closed. Defaults remain unchanged. See `config/collect_hosts.json.example` under kvtest and regression suite `test_collect_targets.py` (synthetic 500-target coverage and mocked Worker/Client ordering).
