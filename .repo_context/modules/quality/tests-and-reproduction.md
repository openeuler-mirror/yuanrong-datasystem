...
Collection transport options are opt-in: `--compress/--no-compress` controls gzip on the transfer archive; `--extract/--no-extract` controls local unpacking. Both Worker and Client support all combinations in full and filtered collection. Omitted options preserve legacy transport paths and defaults. Explicit full transfers reuse the existing file discovery and remote tar; filtered transfers use the Python archive helper. `receive_archive` spools data to disk, safely extracts files or atomically retains `.tar.gz`/`.tar`, and does not replace an existing archive on failed transfer. Client full output/SDK archives use separate names; reproduction config copies remain separate. Validate with `tests/kvtest/tests/python/test_collect_compression.py` and existing collect/deploy regression suites. No service hot path or persisted service format is changed; target compression CPU and local spool storage remain operator-managed costs.
