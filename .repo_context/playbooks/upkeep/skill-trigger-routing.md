...
| `ds-trace-triage` | user asks to parse DataSystem trace packages into normalized evidence and an offline triage report | user asks only what one trace field or log line means |
| `ds-trace-bottleneck-analysis` | user asks for GET TopN bottlenecks, latency-band analysis, or isolated multi-Run control-variable comparison after triage | user asks only about report schema, chart styling, or script internals |
| `ds-trace-numa-analysis` | user asks to correlate WR, inflight, chip/NUMA distribution, or receiver-side NUMA evidence for trace runs | user asks only what one NUMA or WR field means |
