...
The repository keeps brpc and ZMQ as two interchangeable backends selected by `FLAGS_use_brpc` / `DATASYSTEM_USE_BRPC`. They share the same configured worker/master port because `kBrpcPortOffset = 0`; only one backend should bind the endpoint in a process.
