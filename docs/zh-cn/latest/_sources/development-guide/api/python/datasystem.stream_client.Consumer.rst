...
.. py:class:: datasystem.stream_client.Consumer(consumer)
    ...
    .. list-table::
       ...
       * - :doc:`receive <datasystem.stream_client.Consumer.receive>`
         - 消费者接收数据带有订阅功能，接收数据会等待接收expectNum个elements的时候返回成功，或者当超时时间timeoutMs到达返回成功。
       * - :doc:`receive_any <datasystem.stream_client.Consumer.receive_any>`
         - 消费者获取到element后立刻返回。如果没有element，将等待直到超时时间到达。
       * - :doc:`ack <datasystem.stream_client.Consumer.ack>`
       ...
       * - :doc:`close <datasystem.stream_client.Consumer.close>`
...
.. toctree::
    ...
    datasystem.stream_client.Consumer.receive_any
    datasystem.stream_client.Consumer.ack
