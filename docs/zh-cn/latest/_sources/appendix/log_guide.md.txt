...
9. **指标摘要日志（kv_metrics）**：Worker 或 Coordinator INFO 日志中 `metrics_summary` 摘要的 JSON-Lines 并行输出，每行一个完整 JSON 对象，顶层含 `time`/`pod_name`/`cluster_name`，body 与 INFO.log 逐字一致（需 `json_log_monitor`；INFO.log 原 `metrics_summary` 仍由 `log_monitor` 控制）。Worker 输出资源和数据面指标，Coordinator 输出 RPC、Watch 分发及通知指标。
...
| 10 | datasystem_coordinator | `/path/yr_datasystem/logs/datasystem_coordinator.INFO.log` | Coordinator 运行日志；启用 `log_monitor` 时按 `log_monitor_interval_ms` 周期输出 `metrics_summary` |
| 11 | datasystem_coordinator | `/path/yr_datasystem/logs/kv_metrics.log` | Coordinator RPC、Watch 分发及通知指标的 JSON-Lines 摘要；由 `json_log_monitor` 控制 |
