# kubectl Transport Support Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add kubectl transport to deploy.py so kvclient_standalone_test can be deployed to k8s pods.

**Architecture:** Rewrite deploy.py's transport layer. Three methods (`run_on`, `scp_to`, `collect_files`) dispatch by node's `transport` field (ssh/kubectl/localhost). `generate_config()` auto-injects `instance_id`, `nodes`, and `peers` from deploy.json, simplifying config.json to pure runtime params.

**Tech Stack:** Python3, subprocess, kubectl, scp, tar

---

### Task 1: Rewrite deploy.py

**Files:**
- Rewrite: `tests/kvclient_standalone/deploy.py`

The current deploy.py has a missing `scp_to()` method and broken `scp_from()` code. Since kubectl support requires changing method signatures across the entire file (e.g., `run_on` changes from `(host, user, cmd)` to `(node, cmd)`), we rewrite the whole file.

- [ ] **Step 1: Write the complete new deploy.py**
...
class Deployer:
    ...
    def collect_files(self, node, local_dir):
        ...
        try:
            if transport == 'localhost':
                if os.path.exists(tar_remote):
                    shutil.copy2(tar_remote, tar_local)
                else:
                    return
            elif transport == 'kubectl':
                ns = self._namespace(node)
...
Run: `cd tests/kvclient_standalone && python3 deploy.py`
Expected: Usage message printed to stdout

- [ ] **Step 4: Commit**

```bash
git add tests/kvclient_standalone/deploy.py
git commit -m "feat: add kubectl transport support to deploy.py"
```

---

### Task 2: Update config.json.example

Remove `instance_id` and `nodes` fields — these are now auto-injected by deploy.py from deploy.json. The config template becomes a pure kvclient runtime parameters file.

**Files:**
- Modify: `tests/kvclient_standalone/config/config.json.example`

- [ ] **Step 1: Write updated config.json.example**
...
Run: `python3 -c "import json; json.load(open('tests/kvclient_standalone/config/config.json.example')); print('OK')"`
Expected: `OK`
...
git commit -m "feat: remove instance_id and nodes from config template"
```

---

### Task 3: Update deploy.json.example

Add kubectl node example alongside existing SSH nodes.

**Files:**
- Modify: `tests/kvclient_standalone/config/deploy.json.example`

- [ ] **Step 1: Write updated deploy.json.example**
...
Run: `python3 -c "import json; json.load(open('tests/kvclient_standalone/config/deploy.json.example')); print('OK')"`
Expected: `OK`

- [ ] **Step 3: Commit**

```bash
git add tests/kvclient_standalone/config/deploy.json.example
git commit -m "feat: add kubectl node example to deploy.json.example"
```

---

### Task 4: Rebuild output directory

Since deploy.py changed, regenerate the output/ directory.

**Files:**
- Run: `make package` in `tests/kvclient_standalone/`

- [ ] **Step 1: Clean and rebuild output**

Run:
```bash
cd tests/kvclient_standalone && make clean && make package BAZEL_SDK_DIR=/home/duke/code/claude/opt_bazel_build/yuanrong-datasystem/output/cpp
```

Expected: output/ directory rebuilt with updated deploy.py and config files.

- [ ] **Step 2: Verify output contents**

Run: `ls -la tests/kvclient_standalone/output/`

Expected: kvclient_standalone_test binary, deploy.py, test_deploy.sh, lib/, config/

- [ ] **Step 3: Commit**

```bash
git add tests/kvclient_standalone/output/
git commit -m "chore: rebuild output directory with kubectl-enabled deploy.py"
```
