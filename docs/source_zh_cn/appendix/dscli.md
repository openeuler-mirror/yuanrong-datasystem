...
- [dscli安装教程](#dscli安装教程)
    ...
    - [源码编译安装](#源码编译安装)
- [dscli使用教程](#dscli使用教程)
    - [yr-datasystem集群部署](#yr-datasystem集群部署)
...
[![查看源文件](https://Mindspore-website.obs.cn-north-4.myhuaweicloud.com/website-images/master/resource/_static/logo_source.svg)](https://gitee.com/mindspore/yr-datasystem/tree/master/docs/source_zh_cn/appendix/dscli.md)
...
dscli命令集成在 yr-datasystem 的 wheel 包中，因此在环境中安装 yr-datasystem 完成后即可使用 dscli 命令行工具。
...
指定好yr_datasystem以及配套的Python版本，运行如下命令安装yr-datasystem包：
...
pip install https://ms-release.obs.cn-north-4.myhuaweicloud.com/${version}/yr_datasystem/any/yr_datasystem-${version}-cp${py_version}-cp${py_version}-linux_x86_64.whl --trusted-host ms-release.obs.cn-north-4.myhuaweicloud.com -i https://pypi.tuna.tsinghua.edu.cn/simple
```

### 源码编译安装

源码编译安装 yr-datasystem 详细说明请参考：[源码编译方式安装yr-datasystem版本](../getting-started/install.md#源码编译方式安装yr-datasystem版本)。 
...
yr-datasystem集群管理依赖ETCD，在部署yr-datasystem集群前请确保ETCD集群处于可用状态。ETCD部署教程可参考：[ETCD集群部署](../getting-started/deploy.md#安装并部署ETCD)。

#### 单机部署

yr-datasystem单机部署依赖 [dscli start](#dscli-start) 命令：
...
yr-datasystem单机卸载依赖 [dscli stop](#dscli-stop) 命令：
...
通过 [dscli generate_helm_chart](#dscli-generatehelmchart) 命令在当前目录生成 yr-datasystem 的 Helm Chart 包：
...
可通过 `datasystem-helm-chart/datasystem/values.yaml` 配置 yr-datasystem Kubernetes 部署所需的配置项，Kubernetes 配置项详细说明请参考：[yr-datasystem Kubernetes配置项](./k8s_configuration.md)。

配置完成后即可通过如下命令拉起 yr-datasystem 集群：
...
更多 yr-datasystem Kubernetes 部署详细信息请参考：[yr-datasystem Kubernetes部署](../getting-started/deploy.md#yr-datasystem-kubernetes部署)。

更多关于 dscli generate_helm_chart 命令的使用请参考：[dscli generate_helm_chart](#dscli-generatehelmchart)。

### 生成Cpp样例代码

通过 [dscli generate_cpp_template](#dscli-generatecpptemplate) 命令可快速生成 yr-datasystem 的 C++ 样例代码：
...
├── include                 // yr-datasystem SDK头文件
├── kv_cache_example.cpp    // 样例代码
├── lib                     // yr-datasystem SDK动态库及其依赖库
├── README.md               // 说明文档
└── run.sh                  // 编译&运行脚本
```
...
| worker_nodes | list | `"127.0.0.1"` | 部署yr-datasystem的节点 |
...
| 配置项 | 类型 | 默认值 | 描述 |
|-----|------|---------|-------------|
| max_client_num | int | `200` | yr-datasystem单个DaemonSet最大可同时连接的客户端数 |
| shared_memory_size_mb | int | `1024` | yr-datasystem单个DaemonSet可使用的共享内存资源大小（以MB为单位） |
...
| curve_key_dir | string | `"/home/sn/datasystem/curve_key_dir"` | 用于查找 ZMQ Curve 密钥文件的目录，启用 ZMQ 认证时必须指定该路径 |
...
| oc_thread_num | int | `64` | 配置服务端用于处理对象/KV缓存的业务线程数 |
...
| spill_size_limit | string | `"0"` | 配置缓存溢出的最大容量（以字节为单位） |
...
| 配置项 | 类型 | 默认值 | 描述 |
|-----|------|---------|-------------|
| log_dir | string | `"/yr_datasystem/logs"` | 日志目录，服务端产生的日志将保存到此目录中，该路径会被自动挂载到宿主机同名路径下 |
...
| shared_memory_populate | bool | `true` | 是否开启共享内存预热功能，启用该功能可以加速应用运行期间的共享内存拷贝速度，但是在datasystem_worker进程启动时也会由于预热导致启动速度变慢（取决于sharedMemory的配置）。如果开启该功能，'arena_per_tenant'必须设置为1，'enable_fallocate'必须设置为false |
