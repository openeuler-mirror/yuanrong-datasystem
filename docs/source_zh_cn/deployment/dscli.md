...
| log_rate_limit | int | `0` | 每秒最大日志条数（0表示不限速）。超出限制后，INFO、WARNING和ERROR日志将被等间隔采样，日志中标注采样率。FATAL不受限速影响 |
...
| warmup_connection | string | `urma` | 是否在worker启动时预热worker-worker连接。`none` 表示关闭；当前支持 `none`、`urma`，配置为 `urma` 时需要同时开启 `enable_urma=true` |
